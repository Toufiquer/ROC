...
import { getPrivateBatchAddmission, createPrivateBatchAddmission, updatePrivateBatchAddmission, deletePrivateBatchAddmission } from './controller';
...
export async function GET(req: Request) {
  ...
  return safeCall(() => getPrivateBatchAddmission(req));
}
...
export async function POST(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => createPrivateBatchAddmission(req));
}
...
export async function PUT(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => updatePrivateBatchAddmission(req));
}
...
export async function DELETE(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => deletePrivateBatchAddmission(req));
}
