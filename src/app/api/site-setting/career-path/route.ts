...
import { getCareerPath, createCareerPath, updateCareerPath, deleteCareerPath } from './controller';
...
export async function GET(req: Request) {
  ...
  return safeCall(() => getCareerPath(req));
}
...
export async function POST(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => createCareerPath(req));
}
...
export async function PUT(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => updateCareerPath(req));
}
...
export async function DELETE(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => deleteCareerPath(req));
}
