...
import { getSuccessStory, createSuccessStory, updateSuccessStory, deleteSuccessStory } from './controller';
...
export async function GET(req: Request) {
  ...
  return safeCall(() => getSuccessStory(req));
}
...
export async function POST(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => createSuccessStory(req));
}
...
export async function PUT(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => updateSuccessStory(req));
}
...
export async function DELETE(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => deleteSuccessStory(req));
}
