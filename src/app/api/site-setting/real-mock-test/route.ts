...
import { getRealMockTest, createRealMockTest, updateRealMockTest, deleteRealMockTest } from './controller';
...
export async function GET(req: Request) {
  ...
  return safeCall(() => getRealMockTest(req));
}
...
export async function POST(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => createRealMockTest(req));
}
...
export async function PUT(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => updateRealMockTest(req));
}
...
export async function DELETE(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => deleteRealMockTest(req));
}
