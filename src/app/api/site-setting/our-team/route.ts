...
import { getOurTeam, createOurTeam, updateOurTeam, deleteOurTeam } from './controller';
...
export async function GET(req: Request) {
  ...
  return safeCall(() => getOurTeam(req));
}
...
export async function POST(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => createOurTeam(req));
}
...
export async function PUT(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => updateOurTeam(req));
}
...
export async function DELETE(req: Request) {
  const limit = handleRateLimit(req);
  if (limit) return limit;
  return safeCall(() => deleteOurTeam(req));
}
